...
Computation helps explore alternatives and provides evidence for the architect's decisions.
...
**Discussion:** Which campus-delivery choices would change the architecture?

**Suggested answer:** Vehicle type, depot arrangement, loading, and handoff can change function allocation and interfaces. Paint color normally does not under the stated delivery goals, unless an additional requirement makes it consequential.
...
**Topics:** Apollo, decision support, modeling tools, and architectural challenges.
...
**Discussion:** Does entering Earth orbit necessarily mean using EOR?

**Suggested answer:** No. EOR requires rendezvous between vehicles in Earth orbit. A single vehicle can enter orbit without rendezvous, so an orbital maneuver and the EOR mission mode are distinct choices.

**Source:** Crawley, Cameron & Selva (2016), §14.1, pp. 325–326; Fig. 14.1.
:::


## Effects of Mission Mode {.smaller}
...
**Discussion:** Why avoid taking an Earth-entry heat shield to the lunar surface?

**Suggested answer:** It serves a later Earth-return function and adds mass to lunar descent and ascent. Separating vehicles can avoid carrying it through those maneuvers, at the cost of extra interfaces and rendezvous operations.
...
**Discussion:** If capacity does not affect the score, should we remove that decision?

**Suggested answer:** First check whether the metric set captures stakeholder needs. Capacity may be irrelevant within the fixed scope, or its apparent irrelevance may expose an omitted benefit or constraint.
...
**Discussion:** Does choosing LOR leave lunar arrival and departure unconstrained?

**Suggested answer:** No. In this model, LOR requires orbital arrival and departure. The individual decision domains list possibilities; the logical rules determine which combinations are allowed.
...
**Discussion:** Why distinguish CM crew from LM crew?

**Suggested answer:** They describe different vehicle occupancies. A separate lunar module can carry a subset of the command-module crew, but cannot require more people than the mission provides.
...
**Discussion:** Give a counterexample to ‘Earth orbit implies EOR.’

**Suggested answer:** A single spacecraft can enter Earth orbit and later depart without meeting another spacecraft. EOR implies the orbital maneuver in this model, but the converse does not follow.
...
**Discussion:** How can an assignment with two CM crew and three LM crew be repaired?

**Suggested answer:** Reduce LM crew to one or two, or increase CM crew to three. The repaired assignment must also satisfy the mission-mode and vehicle-existence rules.
...
**Discussion:** When is a penalty more informative than excluding a candidate?

**Suggested answer:** When the candidate is feasible but unattractive on a tradable concern. A penalty or explicit metric preserves the tradeoff; a hard constraint is appropriate for a required condition.
...
**Discussion:** Why can't we multiply option counts to obtain the feasible count directly?

**Suggested answer:** That product counts all assignments before coupled constraints. Apply the rules and remove invalid combinations; also distinguish distinct assignments from distinct physical architectures or metric points.
...
**Discussion:** What might a mass-versus-success plot omit?

**Suggested answer:** Schedule, scientific return, abort capability, and development effort are possible omissions. Whether they matter depends on the agreed goals; a two-axis plot does not make them disappear.
...
**Discussion:** Why can a small payload-mass reduction have a larger launch-mass effect?

**Suggested answer:** Payload must be accelerated along with the propellant and supporting structure needed for later maneuvers. Removing mass from a downstream stage can reduce upstream propulsion demands as well.
...
**Discussion:** Can we read a maneuver's success factor as a measured overall mission reliability?

**Suggested answer:** No. It is an input to the chapter's simplified model. Overall scoring combines the factors under stated assumptions and should not be confused with observed historical reliability.
...
**Discussion:** What could invalidate multiplying separate success factors?

**Suggested answer:** Dependence or common-cause failures can make the product inappropriate unless the factors are correctly conditioned. Identify what each factor means and which independence assumptions the calculation uses.
...
**Discussion:** What does reducing LM crew to one do in the worked factor model?

**Suggested answer:** It multiplies 0.8212 by 0.90, giving about 0.7391: a 10% relative decrease, or about 8.21 percentage points. Check feasibility before evaluating the score.
...
**Discussion:** Must the point with highest modeled success be selected?

**Suggested answer:** No. It may require substantially more mass or perform poorly on omitted goals. The tradespace reveals tradeoffs; stakeholder preferences and model confidence inform selection.
...
**Discussion:** What evidence would help choose among the highlighted candidates?

**Suggested answer:** Compare schedule feasibility, abort behavior, development demands, and uncertainty in the modeled scores. Investigate whether the apparent performance differences are large enough to support the decision.
...
**Discussion:** Turn ‘we care about reliability’ into a decision statement.

**Suggested answer:** ‘Choose single or redundant sensing’ identifies alternatives. Reliability is then a metric used to compare them; it is not itself the architectural choice.
...
**Discussion:** What is missing when a team immediately votes on its first two ideas?

**Suggested answer:** It may skip understanding the problem and deliberately developing alternatives. A useful process also reviews the result and revisits the framing when new evidence appears.
...
**Discussion:** Is choosing aircraft size always a programmed decision?

**Suggested answer:** No. Established calculations may support part of it, while market needs, context, and program goals remain unfamiliar. Decisions can combine programmed and non-programmed aspects.
...
**Discussion:** When could reusing a previous decision method mislead us?

**Suggested answer:** When the context, available alternatives, goals, or governing assumptions have changed. Reuse the method only after checking which parts of the earlier problem still apply.
...
**Discussion:** Classify listing fuels, linking LOR to fuel, scoring success, and plotting results.

**Suggested answer:** They are representing, structuring, simulating, and viewing, respectively. Each answers a different question, and conclusions should remain traceable across the layers.
...
Evaluating an architecture also requires feasibility rules, metrics, and a model of stakeholder preferences.
...
**Discussion:** What changes in the functional model when there is no separate LM?

**Suggested answer:** The required lunar landing and ascent functions still need enabling form, but they are allocated differently. Remove the absent object and revise the processes and interfaces that depended on it.
...
**Discussion:** Does a blank in a constraint DSM prove two decisions are independent?

**Suggested answer:** No. It means no direct relationship of the represented type. They may interact through a performance metric, an indirect path, or a relationship omitted from that matrix.
...
**Discussion:** Can CM crew and LOR be related without a direct DSM mark?

**Suggested answer:** Yes. A path through LM crew links them in the modeled constraint structure. Distinguish direct adjacency from indirect dependence when interpreting the matrix.
...
**Discussion:** How can decisions interact through mass without sharing a logical constraint?

**Suggested answer:** A fuel choice can change the mass consequences of a crew or mission-mode choice while all combinations remain logically valid. Feasibility coupling and metric coupling are different relationships.
...
**Discussion:** What does DSM clustering suggest for organizing a trade study?

**Suggested answer:** Strongly coupled decisions should be examined together. Check which relationship types the matrix includes before treating separated groups as suitable for independent study.
...
**Discussion:** How do a decision node and a chance node differ?

**Suggested answer:** The decision maker chooses an alternative at a decision node. A chance node represents an uncertain outcome; evaluation must use the available information and scenario probabilities.
...
**Discussion:** Why does the first Apollo tree lose one of four possible prefixes?

**Suggested answer:** EOR=yes with direct Earth launch violates the model's implication. Pruning removes that invalid prefix and all of its downstream extensions.
...
**Discussion:** Why is LOR=yes with direct lunar arrival absent?

**Suggested answer:** It violates the orbital-arrival requirement associated with LOR. The fifteen mission-mode leaves count only that part of the decision model, not all crew and fuel assignments.
...
The number of paths and the cost of evaluating each one determine the computational effort.
...
**Discussion:** How many assignments do twenty independent binary decisions create?

**Suggested answer:** There are 2^20 = 1,048,576 before constraints or scenarios. A short list of decisions can therefore produce a tree too large for convenient explicit inspection.
...
Normalize mass and success probability and define stakeholder preferences before combining them.
...
**Discussion:** What does increasing the mass-utility weight do?

**Suggested answer:** It increases mass utility's relative contribution to the combined score, with an effect that depends on normalization and utility shape. It does not change the architectures' physical performance.
...
**Discussion:** Why can't we simply choose the leaf with utility 90?

**Suggested answer:** That leaf includes a favorable outcome outside the decision maker's control. Compare the alternatives across their possible outcomes rather than selecting the scenario as though it were an action.
...
**Discussion:** Which alternative wins with equally likely scenarios?

**Suggested answer:** EU(A)=60 and EU(B)=65, so B wins under the given utility model. Compute expectations at chance nodes, then choose at decision nodes using the correct information order.
...
**Discussion:** What if the optimistic probability could plausibly lie between 0.4 and 0.8?

**Suggested answer:** The preferred alternative changes at p=0.60, so that range crosses the decision threshold. Better probability evidence or a stated robustness preference could materially affect the choice.
...
- **Evaluation effort:** estimating each architecture's outcome under each scenario may require expensive simulation.
...
- Clarify the scope of the book's independence criticism: simple independent-factor evaluation is restrictive. General decision trees can represent conditional probabilities and dependent results by expanding the histories, but this can be unwieldy. Dependence is not mathematically forbidden in every tree.
...
**Discussion:** Which tool can reveal shared dependencies before building a large tree?

**Suggested answer:** A decision DSM can expose coupled choices. It does not replace scenario evaluation, but it helps structure the problem and identify where a simple tree may obscure dependence.
...
**Discussion:** How would you combine the chapter's tools in one workflow?

**Suggested answer:** List alternatives in a decision representation, use a DSM to expose coupling, generate feasible assignments, evaluate outcomes, and inspect the tradespace or appropriate decision tree. Iterate when the model proves incomplete.
...
**Discussion:** Classify ‘the sensor may fail’ and ‘the product should be intuitive.’

**Suggested answer:** The first can motivate an uncertainty model. The second requires clarification of the desired experience and how it will be evaluated. More simulation alone does not resolve unclear meaning.
...
**Discussion:** Can stakeholders agree on mass and disagree on value?

**Suggested answer:** Yes. They may value cost, reliability, convenience, or risk differently. Physical metric agreement does not establish common preferences or a unique utility function.
...
Even a few strongly connected decisions can create many combinations and system-level effects.
...
- Reconnect to emergence in earlier chapters: system value is produced by the interaction of selections, not only the independent quality of the selected parts.

**Discussion:** Why doesn't choosing each lightest component guarantee the lightest system?

**Suggested answer:** Component choices affect interfaces, supporting structure, power, and operation. Additional integration resources can outweigh local savings, so evaluate the assembled architecture.
...
**Discussion:** What explanation should a support tool give when rejecting a candidate?

**Suggested answer:** Identify the violated constraint or the assumptions and preference judgments behind its low score. A useful explanation lets the architect audit and, when justified, revise the model.
...
**Discussion:** Give a complete possible answer for the campus-delivery exercise.

**Suggested answer:** Assume automated loading requires a compatible ground robot, and unstaffed handoff requires a locker. Compare delivery time and cost per parcel. In OPM terms, Transporting changes Parcel location using Vehicle; vehicle and loading choices are coupled.
...
- Explain that “unstaffed” is contextual here; if staffing becomes a decision, add it explicitly rather than hide it in a rule. The staffed desk / secure locker alternatives already carry an assumed operating concept.
...
**Discussion:** What changes if a compatible automated drone loader becomes available?

**Suggested answer:** Revise the compatibility rule and reconsider previously excluded combinations. Retain other constraints and re-evaluate performance; a new feasible option is not automatically the preferred one.

**Source:** Crawley, Cameron & Selva (2016), §§14.2, 14.4–14.6; classroom application.
:::


## Chapter 14 Summary {.smaller}
...
- Record the evidence and judgments behind each assessment so others can examine the reasoning.
...
**Discussion:** Give one Apollo example for each decision-support layer.

**Suggested answer:** Represent fuels as alternatives; structure the LOR and lunar-maneuver dependency; simulate mass and modeled success; view the resulting tradespace. Each layer should retain links to the assumptions that produced it.
...
2. What information does a blank cell in a constraint DSM provide? Which conclusions would require additional information?
