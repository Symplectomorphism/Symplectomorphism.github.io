...
## Box 5.1 · Definition: Function {.smaller}
...
::: {.side-caption}
Source: Crawley, Cameron & Selva (2016), Box 5.1.
:::

::: {.notes}
- The definition concerns activity, not the static existence of parts. Performance describes how well the activity is carried out.
- Form executes function as its instrument, but function is an attribute of the product or system, not an object or part.
- The externally visible function emerges from interactions among entities and their internal functions.

**Discussion:** Why does listing a pump's components not fully define its function?

**Suggested answer:** Components describe form. The pump's function concerns the process that changes the operand, such as pressurizing water, and the performance of that process.
:::
...
## Function as Process and Operand {.smaller}

Function joins an **action** to the **object acted upon**:

| Process | Operand | Function |
|---|---|---|
| Housing | Resident | Housing a resident |
| Instructing | Passenger | Instructing a passenger |

::: {.fragment .callout-tip}
**Function = process + operand.** Form supplies the instrument that enables the process.
:::

::: {.side-caption}
Examples and relation from Crawley, Cameron & Selva (2016), §5.2, pp. 98–99.
:::

::: {.notes}
- The relation is introduced in the book before the operand and process definitions. It is a way to keep the action and what changes together.
- The resident and passenger may arrive only when the system is used; the house and instruction card are instruments supplied with the system.
- Next, distinguish operands from objects of form, then define process as the transformation undergone by an object.

**Discussion:** In a pump, what are the process and operand?

**Suggested answer:** Pressurizing is the process; water is the operand whose pressure changes. The pump is the instrument of form.
:::

## Box 5.2 · Definition: Operand {.smaller}
...
::: {.side-caption}
Source: Crawley, Cameron & Selva (2016), Box 5.2.
:::

::: {.notes}
- An operand is an object, but its role differs from form: it is acted upon by a process, and need not be supplied with the system.
- It can already exist, appear during operation, change state, or be consumed. A resident and a passenger are operands of housing and instructing, respectively.
- The architect often does not control the operand; nevertheless its possible states matter to the architecture.

**Discussion:** Does the operand have to exist before operation begins?

**Suggested answer:** No. A process may create the operand, or act on one that already exists.
:::

## Box 5.3 · Definition: Process {.smaller}
...
::: {.side-caption}
Source: Crawley, Cameron & Selva (2016), Box 5.3.
:::

::: {.notes}
- A process is transient and dynamic. It occurs over time, unlike an object, which can have stable existence.
- Housing changes a resident from unhoused to housed; instructing changes a passenger's knowledge.
- Keep roles distinct: in “a drill makes a hole in a plate,” drilling is the process, the plate is the operand, and the drill is the instrument.

**Discussion:** What changes if the drill bit itself wears down?

**Suggested answer:** The bit is then also an operand of a wear process. Operand and instrument describe roles in particular processes, not permanent object categories.
:::
...
**Trace a handoff:** In the high-v column, Accelerating creates the operand that Diffusing consumes. Figure 5.13 later draws this handoff as an OPM diagram.
...
::: {.notes}
First read the create entries to identify the internal processes; compare all four rows with Figure 5.13 later. Each of the three internal flow columns has a producer and consumer. External low-pressure flow enters through Inflowing; external high-pressure flow leaves through Outflowing.
...
::: {.side-caption}
Source: Figure 5.17, a projection of the flow-through pump model. The next slides develop Box 5.8's matrix construction.
:::
...
- `PO`: m × n; `OP`: n × m, constructed on the next slide.
...
- Last bullet: **projections** (Figure 5.17 here; Box 5.8 above): simplify process-operand diagrams by projecting onto just the processes, leaving shared operands implicit. Analogous to DSM-projection techniques elsewhere in the book: this matrix-based machinery reappears later, particularly Ch. 13's decomposition discussion
