...
## Figure 5.5 · SysML State-Machine Example {.smaller .chapter05-detail}

::: {.columns}
::: {.column width="55%"}
![](assets/chapter05/fig5_5_robot_state_machine.png){height=460px fig-alt="Robot controller state machine with waiting, moving, turning, and shutdown behavior."}
:::
::: {.column width="45%"}
- States and transitions emphasize **behavior over time**.
- Events and conditions determine which transition occurs.
- Trace Moving forward → Sensor hit → Turning → Moving forward.
- This is a notation comparison; the main course models use **OPM**.
:::
:::

::: {.side-caption}
Source: Figure 5.5. The timeout condition provides a different path toward shutdown.
:::

::: {.notes}
The robot controller illustrates that states and transitions describe behavior without showing all objects and their enabling form on the same diagram. Do not confuse the rounded state boxes here with the process ovals of OPM; read each notation using its own grammar.

**Discussion:** Does a state-machine diagram alone identify which physical actuator enables motion?

**Suggested answer:** No. That allocation requires other model information. OPM's canonical pattern can put the instrument, process, and operand together.

[Sources]
- Crawley, Cameron & Selva, *System Architecture* (2016), Ch. 5, p. 102, Fig. 5.5.
:::

## Figure 5.6 · State Change versus OPM Context {.smaller .chapter05-detail}

::: {.columns}
::: {.column width="30%"}
![](assets/chapter05/fig5_6_instruction_states.png){height=310px fig-alt="Instruction state transition from Does not know through Informing to Knows."}

**Figure 5.6:** state transition.
:::
::: {.column width="70%"}
![](assets/chapter05/fig5_4_canonical_with_states.jpg){width=100%}

**Figure 5.4:** the canonical OPM model includes the passenger, knowledge states, Informing, and the instruction card.
:::
:::

Ask both: **What changes?** And **whose state changes, enabled by what?**

::: {.side-caption}
Sources: Figures 5.4 and 5.6. The state-only view omits the traveler and instruction-card objects.
:::

::: {.notes}
Both depictions concern the same intended knowledge change. The OPM view names the operand and enabling instrument explicitly; the simple state-machine view focuses on the transition. Supplemental comparison only: students can follow the main chapter entirely in OPM.

**Discussion:** What should be restored if we use the state-only sketch to allocate function to form?

**Suggested answer:** Identify the traveler whose knowledge changes and the instruction card or other instrument enabling Informing. A transition label alone does not establish that allocation.
