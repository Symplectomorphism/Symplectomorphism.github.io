...
$$
\begin{aligned}
\mathcal{L}\{e^{(\sigma_{0}+j\omega_{0})t}u_{s}(t)\}
&= \int_{0}^{\infty} e^{-st}e^{(\sigma_{0}+j\omega_{0})t}\,dt \\
&= \int_{0}^{\infty} e^{-[s-(\sigma_{0}+j\omega_{0})]t}\,dt \\
&= \left.
\frac{e^{-[s-(\sigma_{0}+j\omega_{0})]t}}
{-[s-(\sigma_{0}+j\omega_{0})]}
\right\vert_{0}^{\infty} \\
...
{-[s-(\sigma_{0}+j\omega_{0})]}
\right\vert_{0} \\
&= \underbrace{\lim_{t\rightarrow\infty}
\frac{e^{-(s-\sigma_{0})t+j\omega_{0}t}}
{-[s-(\sigma_{0}+j\omega_{0})]}}
_{=0\text{ for }\sigma>\sigma_{0}}
+ \frac{1}{s-(\sigma_{0}+j\omega_{0})}.
\end{aligned}
$$
...
:   $\mathcal{L}
    \left\{\dfrac{d}{dt}f(t)\right\}=sF(s)-f(0)$
    ...
    **Proof:** By definition,
    ...
    $$
    \mathcal{L}\left\{\dfrac{d}{dt}f(t)\right\}
    \triangleq \int_{0}^{\infty}e^{-st}f^{\prime}(t)\,dt.
    $$

    Integrate by parts with

    $$
    u=e^{-st},\quad dv=f^{\prime}(t)\,dt,
    \qquad du=-se^{-st}\,dt,\quad v=f(t).
    $$

    Therefore,

    $$\begin{aligned}
    \int_{0}^{\infty}e^{-st}f^{\prime}(t)\,dt
...
Complex number: $$c=a+jb,\qquad a,b\in
\mathbb{R}
...
$$
\left(\frac{c_{1}}{c_{2}}\right)^{\ast}
=\frac{c_{1}^{\ast}}{c_{2}^{\ast}},
\qquad
(c_{1}+c_{2})^{\ast}=c_{1}^{\ast}+c_{2}^{\ast}.
$$
...
\!s+(A_{0}+A_{1})s^{2}.$$ This results in $$A_{0}=1,\qquad A_{1}=-1,\qquad A_{2}=-\frac{A_{0}+A_{1}/2}{\sqrt{3}
...
{2}+j\frac{1}{2\sqrt{3}}\qquad\Longrightarrow\beta_{1}^{\ast}
...
/6)u_{s}(t)\qquad\text{Euler's formula}\\
...
Use long division: $$\begin{array}{r|l}
& s-5\\
s^{2}+5s+4 & \overline{s^{3}+0s^{2}+0s+0}\\
& \underline{s^{3}+5s^{2}+4s}\\
& -5s^{2}-4s\\
& \underline{-5s^{2}-25s-20}\\
& 21s+20
...
::: {.columns .half-plane-slide}
::: {.column width="54%"}
Let $s=\sigma+j\omega$, so that
$$
...
Let $F(s)=
\mathcal{L}
\{f(t)\}$ be strictly proper and rational. Then
$$
f(t)\rightarrow0\quad\text{as}\quad t\rightarrow\infty
$$
if and only if *all* the poles of $F(s)$ are in the open left half-plane.
...
::: {.column width="42%"}
::: {.center}
![](assets/chapter02/open-left-half-plane.svg){width="96%" fig-alt="The open left half-plane"}
:::
:::
:::
