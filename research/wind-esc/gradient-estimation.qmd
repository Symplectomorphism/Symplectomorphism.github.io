...
being wrong by half a unit in lambda costs about one percent of the power, so
...
![](assets/piesc.svg){.fig-tall fig-alt="Comparison: LP-ESC demodulates a dither; LP-PIESC fits a two-parameter model by recursive least squares. Three result cards: smaller dither, energy loss cut from 14.2 to 0.3 percent, and 8.9 percent farm power gain in a wind tunnel."}
...
of 0.3 percent against 14.2, with a dither a quarter the size. That is why the
...
in k: 7.7 percent low, which is 0.19 in lambda.
...
12 and 86 percent of it exceeds 2. The boundary lands between 8.7 and 13.3, and
any further phase in the loop, rotor lag or the blanked 27 percent of each
...
::: {.notes}
Without it there is no way to know whether LP-PIESC is within twenty percent of
