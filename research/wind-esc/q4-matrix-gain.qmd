...
![](assets/zap-relerr.svg){.fig-tall fig-alt="Relative error against distance from the peak, on log axes. The gradient's relative error rises steeply as the peak is approached; the curvature's is a flat line. They cross at about three percent from the peak."}
...
1.78. Flat noise would still give a crossover, at 1.7 percent.
...
Two honest caveats on it. The 20 percent target is a judgement, chosen because
the plant curvature itself swings 20 percent across the terminal scatter, so
...
**illustration**, since its loop gain was also far too high and it saturated, so no
