...
format:
  ...
  revealjs:
    ...
    lang: en
    footer: "[&#8592; Research Home](../index.qmd) &middot; Nonprehensile Manipulation"
---
...
Gap-greedy is the named baseline, not a stand-in for "any old order". It scores an
action by the signed gaps of the contacts that action *adds*: $-\sum_{c \in \Delta^+}
\varphi_c(\chi)/\ell$, where $\varphi_c(\chi)$ is how far apart the two features of
contact $c$ still are at the current pose. "Closest" is that gap and nothing else —
read off the pose in front of you, never from where the plan needs to go.

Two consequences. A break or a transport adds no contact, so the sum is empty and
it scores exactly $0$; every make with a real gap to close scores negative. So
gap-greedy prefers changing nothing, and among makes takes whichever is already
nearly touching. Hence the two hops: at the root it takes the floor vertex
$g_v$, already at $\varphi = 0$, over the still-distant riser $s$; after landing it
takes a transport or the release of $s$ over sliding the disk onto the new bottom
face. Both are actions whose payoff arrives several steps later, which is exactly
what a gap read at the current pose cannot see.
:::
...
## The contact graph: one vertex per contact {.smaller}

![](assets/graph.svg){.fig-tall fig-alt="Six contacts as vertices, with m-x and g filled to show the marking sigma-zero, joined by transfer-adjacent, shared-object-face, and exclusion edges."}

::: claim
**A marking is a subset of these vertices** — the two filled are $\sigma_0$. So
a vertex of the marking graph is one on/off setting of *this* picture, and the
walk is it lighting up and going dark. We **search** over markings; the network
**runs here**, and returns which marking-graph edge to take next.
:::

::: {.notes}
Why not run the network on the marking graph itself, since that is what we
search? Two reasons. It has $2^{|C|}$ vertices, so there is nothing to embed.
And its shape carries no information about the scene — it is the Boolean cube
with the excluded subsets deleted, which is the same object for every task.

This graph is the opposite: $|C|$ vertices, built once, and its edges are
physical statements about *pairs of contacts* — same body pair with adjacent
features, same face of the moving object against different environment bodies,
mutually impossible. That is the part with any chance of transferring to a
scene the network has not seen. The shared-face edge is the one that is not
standard, and the next slide shows what it buys.

Concretely: the marking is the network's *input*, written on these vertices as
on/off; a score per available action is its *output*.
:::
