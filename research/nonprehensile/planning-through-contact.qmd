...
::: claim
Carrying unmade contacts in the state is the one structural difference —
...
the limitation: vertices are modes, enumerated up front.

Either is a good discrete object for contacts you already have. Neither can
express "go and establish contact with the riser", which is the decision this
task needs while the box is still on the floor.
:::
...
Mid-topple that is the riser alone, and a single contact can balance gravity
only if its line of action passes through the centre of mass — here it does not.
The figure shows the second, independent reason: the cone about a horizontal
normal contains no vertical force. That second argument uses the report's
convention that the normal is the environment feature's, which at a corner
contact is worth pinning down.
...
![](assets/search.svg){fig-alt="A search from start to goal: four candidates tried at the first node, three at the second, two at the third, with the rejections marked, totalling nine solver calls for a three-action plan."}

::: claim
Nothing so far is learned. It runs, it is sound, and its **only** inefficiency
is the order in which it hands candidates to $\Lambda$.
:::

::: {.notes}
Worth pausing on, because it decides how the rest should be read.
...
no training data, no teacher. Run it and it either returns a certified
trajectory or exhausts its budget.

Its weakness is narrow and specific. At every node it has a list of admissible
actions and no principled order for them, so it spends calls on rejections. In
the sketch, nine calls buy a three-action plan. That gap, and nothing else, is
what a learned layer is allowed to attack.
:::
